...
export default async function Projects({searchParams}:{searchParams:Promise<Record<string,string>>}){
  ...
  return <><Header/><main className="projects-page"><section className="projects-banner" style={header?.image?{backgroundImage:`url(${header.image})`}:undefined}><div className="projects-banner-shade"/><div className="shell"><p>Our Portfolio</p><AnimatedHeading as="h1">{header?.title||"Projects"}</AnimatedHeading><span>{header?.body||"Distinctive addresses, thoughtfully created for modern life."}</span></div></section><section className="projects-filter-section"><div className="shell"><ProjectFilters locations={areas.map(item=>item.name)} propertyTypes={propertyTypes.map(item=>item.name)}/></div></section><section className="projects-body"><div className="shell"><div className="projects-heading"><div><p>Explore our work</p><AnimatedHeading>Signature Projects</AnimatedHeading></div><span>{count} projects</span></div>{items.length?<ProjectGrid projects={items}/>:<div className="empty"><h3>No projects found</h3><p>Try another status, type, location, size, or project name.</p></div>}</div></section></main><Footer/></>;
}
